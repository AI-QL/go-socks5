...
func (d *Dialer) do(ctx context.Context, cmd uint8, address string) (net.Conn, error) {
	if d.IsResolve {
...
func (d *Dialer) connect(ctx context.Context, conn net.Conn, cmd uint8, address string) (net.Conn, error) {
	if d.Timeout != 0 {
...
func (d *Dialer) connectAuth(conn net.Conn) error {
	...
	if uint8(header[1]) == noAcceptable {
		return fmt.Errorf("no acceptable authentication methods %d", uint8(header[1]))
	}
	switch uint8(header[1]) {
	default:
		return fmt.Errorf("authentication method not supported %d", uint8(header[1]))
	case noAuth:
	case userAuth:
		if d.Username == "" {
...
func (d *Dialer) connectCommand(conn net.Conn, cmd uint8, address string) (net.Addr, error) {
...
func (d *Dialer) readReply(conn net.Conn) (net.Addr, error) {
	...
	if uint8(header[1]) != successReply {
		return nil, fmt.Errorf("unknown error %s", Reply2String(uint8(header[1])))
	}
